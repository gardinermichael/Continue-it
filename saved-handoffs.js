...
function reusableCaptureWarnings(handoff) {
  const generationWarningPattern = /AI summarization failed|AI regeneration failed|Summary generated by|Server API|Chrome built-in AI|API key|selected AI setting|quota|model/i;
  return (handoff?.warnings || []).filter((warning) => !generationWarningPattern.test(warning));
}
...
async function regenerateSession(session, mode, statusRoot, triggerButton) {
  ...
  try {
    ...
    const warnings = reusableCaptureWarnings(base);
    let summarySource = summarySourceForMode(mode);
    let aiMode = mode;
    if (mode !== ai.AI_MODES.none) {
      ...
      if (!result.summary) {
        const reason = result.error || "The selected AI setting did not return a summary.";
        warnings.push(`AI regeneration failed; local summary was saved instead: ${reason}`);
        summarySource = "Local fallback after AI failure";
        aiMode = "failed";
        setRegenerationProgress(statusRoot, 64, "AI failed; saving local fallback...");
        setStatus(statusRoot, "AI regeneration failed, so a local fallback handoff will be saved.", "pending");
      } else {
...
function renderVariant(variant, index = 0) {
  ...
  const embeddedChunkCount = packageInfo?.recommendedAdvance || 0;
  const followUpChunkPrompts = packageInfo?.chunkPrompts?.slice(embeddedChunkCount) || [];
  const transferSetupPrompt = embeddedChunkCount ? packageInfo?.recommendedInsertPrompt : packageInfo?.starterPrompt;
  ...
  if (handoff) {
    ...
    if (usesStagedChunks) {
      ...
      chunkNote.textContent = embeddedChunkCount
        ? `Chunks are only needed when the full transcript is too large for one target chat. The first prompt gives the receiving AI the summary and starts the transfer, including transcript chunk 1/${packageInfo.chunkCount}; then send the remaining ${followUpChunkPrompts.length} transcript chunk${followUpChunkPrompts.length === 1 ? "" : "s"} in order if you need the complete raw transcript too.`
        : `Chunks are only needed when the full transcript is too large for one target chat. The first prompt gives the receiving AI the summary and starts the transfer; then send ${packageInfo.chunkCount} transcript chunks in order if you need the complete raw transcript too.`;
      ...
      firstPromptTextarea.value = transferSetupPrompt;
      firstPromptActions.className = "actions";

      addButton(firstPromptActions, "Copy prompt", "secondary", async () => {
        const copied = await shared.copyText(transferSetupPrompt);
      ...
      if (!followUpChunkPrompts.length) {
        const completeNote = document.createElement("p");
        completeNote.className = "summary-text";
        completeNote.textContent = "The recommended prompt already includes all transcript chunks for this handoff.";
        chunkSection.appendChild(completeNote);
      }

      followUpChunkPrompts.forEach((chunkPrompt, offset) => {
        const index = embeddedChunkCount + offset;
